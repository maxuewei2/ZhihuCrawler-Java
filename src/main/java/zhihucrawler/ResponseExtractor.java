            ...
            while (true) {
                ...
                try {
                    if (type.equals("info")) {
                        ...
                        Map<String, Object> info = util.loadJsonString(body, new TypeReference<>() {});
                        //根据info数据构造其他数据请求
                    ...
                    if (data.size() == 0) {
                        if (tryCount < maxEmptyTry) {
                            requestQueue0.put(new RequestNode(user, type, url, id, oldTotalRequestNum, tryCount + 1, requestID));
                            continue;
                        } /*else {
